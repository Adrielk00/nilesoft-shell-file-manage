menu(where=sel.count>0 type='file|dir|drive|namespace|back' mode="multiple" title='Manage files' image=\uE253)
{
	menu(separator="after" title='Copy path' image=icon.copy_path)
	{
		item(where=sel.count > 1 title='Copy (@sel.count) items' cmd=command.copy(sel(false, "\n")))
		item(mode="single" title=@sel.path tip=sel.path cmd=command.copy(sel.path))
		item(mode="single" type='file' find='.lnk' title='Open target in new window' cmd-ps=`-NoProfile -Command $d = '@sel.lnk.dir'; if ($d) { try { Start-Process explorer.exe -ArgumentList $d } catch { } }`)
	}

	item(mode="single" type="file" title="Change extension" image=\uE0B5
		cmd=if(input("Change extension", "New extension"),
		io.rename(sel.path, path.join(sel.dir, sel.file.title + "." + input.result))))

	menu(separator="after" image=\uE290 title='Select')
	{
		...
		item(title="Select none" image=icon.select_none cmd=command.select_none)
	}

	menu(title='Rename' type='file|dir' where=sel.count>0 image=image.mdl(\uE0B5))
	{
		item(title='Clean up name' image=\uE0CE
		...
		item(title='Replace spaces with "_"'
		...
		item(title='lowercase' image=image.mdl(\uE84A)
		...
		item(title='UPPERCASE' image=image.mdl(\uE84B)
			cmd-ps=`-NoProfile -Command $paths = '@(sel(false, "|"))'.Split('|'); foreach ($p in $paths) { $p = $p.Trim().Trim('"'); if (-not $p) { continue }; if (-not (Test-Path -LiteralPath $p)) { $ppar = Split-Path $p -Parent; $pn = ($p -replace '\s+', ' '); if ($ppar -and (Test-Path -LiteralPath $ppar)) { $pf = Get-ChildItem -LiteralPath $ppar -Force -ErrorAction SilentlyContinue | Where-Object { ($_.FullName -replace '\s+', ' ') -eq $pn } | Select-Object -First 1; if ($pf) { $p = $pf.FullName } } }; if (-not (Test-Path -LiteralPath $p)) { continue }; $f = Get-Item -LiteralPath $p; $n = $f.Name.Trim().ToUpperInvariant(); $n = $n -creplace '\\','_' -creplace '/','_' -creplace '<','_' -creplace '>','_' -creplace ':','_' -creplace '\u0022','_' -creplace '\|','_' -creplace '\?','_' -creplace '\*','_'; while ($n.Length -gt 0 -and ($n.EndsWith(' ') -or $n.EndsWith('.'))) { $n = $n.Substring(0, $n.Length - 1) }; if ($n.Length -lt 1) { $n = '_' }; $final = $n; if ($f.Name -ceq $final) { continue }; $orig = $f.Name; $src = $f.FullName; $dir = Split-Path $f.FullName -Parent; if (-not $dir) { continue }; $origFull = Join-Path $dir $orig; $destFull = Join-Path $dir $final; if ($f.Name -eq $final -and $f.Name -cne $final) { $tmp = '__ns_' + (New-Guid).ToString().Replace('-','') + $f.Extension; $mid = Join-Path $dir $tmp; try { Rename-Item -LiteralPath $src -NewName $tmp -ErrorAction Stop; Start-Sleep -Milliseconds 200; Move-Item -LiteralPath $mid -Destination $destFull -Force -ErrorAction Stop } catch { if (Test-Path -LiteralPath $mid) { try { Move-Item -LiteralPath $mid -Destination $origFull -Force } catch { } } } } else { try { Rename-Item -LiteralPath $src -NewName $final } catch { } } }`)

		item(title='Strip accents and diacritics' image=image.mdl(\uE8C1)
		...
		menu(title='Numbering')
		{
			item(title='Auto prefix (0001-)' image=image.mdl(\uF146)
			...
			item(title='Remove numbering' image=image.mdl(\uE894)
	...
	{
		item(title="Hidden files" image=inherit cmd='@command.togglehidden')
	...
	menu(mode="single" type='back' expanded=true)
	{
		...
		{
			item(title='Date' cmd-ps=`-NoProfile -Command $parent = '@sel.dir'; $base = Get-Date -Format 'yyyy-MM-dd'; $candidate = $base; $i = -1; while (Test-Path -LiteralPath (Join-Path $parent $candidate)) { $i = $i + 1; if ($i -gt 10000) { exit }; $num = ''; if ($i -lt 100) { $num = $i.ToString().PadLeft(2, '0') } else { $num = $i.ToString() }; $candidate = $base + ' (' + $num + ')' }; New-Item -ItemType Directory -Path (Join-Path $parent $candidate) | Out-Null`)
			item(title='Date and time' cmd-ps=`-NoProfile -Command $parent = '@sel.dir'; $base = Get-Date -Format 'yyyy-MM-dd HH.mm'; $candidate = $base; $i = -1; while (Test-Path -LiteralPath (Join-Path $parent $candidate)) { $i = $i + 1; if ($i -gt 10000) { exit }; $num = ''; if ($i -lt 100) { $num = $i.ToString().PadLeft(2, '0') } else { $num = $i.ToString() }; $candidate = $base + ' (' + $num + ')' }; New-Item -ItemType Directory -Path (Join-Path $parent $candidate) | Out-Null`)
		...
		menu(title='New file' image=icon.new_file)
		{
			...
			menu(title='General')
			{
				...
				item(title='MD' cmd=io.file.create('@(fdh).md', "# Title\n\nContent\n"))
				item(title='CSV' cmd=io.file.create('@(fdh).csv', "column1,column2,column3\n"))
			}

			menu(title='Web')
			{
				item(title='HTML' cmd=io.file.create('@(fdh).html', "<!DOCTYPE html>\n<html lang=\"en\">\n<head>\n\t<meta charset=\"utf-8\">\n\t<title></title>\n</head>\n<body>\n\n</body>\n</html>\n"))
				item(title='CSS' cmd=io.file.create('@(fdh).css', "/* Styles */\n\n"))
			...
			menu(title='System')
			{
				item(title='INI' cmd=io.file.create('@(fdh).ini', "[section]\nkey=value\n"))
				item(title='REG' cmd=io.file.create('@(fdh).reg', "Windows Registry Editor Version 5.00\n\n"))
			}

			menu(title='Config')
			{
				item(title='.env.example' cmd=io.file.create('@(fdh).env.example', "# Environment variables (copy to .env and fill in)\n#\n# EXAMPLE=value\n"))
			}
		}
	}
